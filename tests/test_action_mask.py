...
def test_guided_trade_mask_blocks_trades_when_immediate_build_available():
    ...
    # Can build a road immediately: should prefer build over proposing player trade.
...
def test_full_trade_mode_keeps_propose_trade_legal():
    ...
    mask = env.action_mask()
    legal_kinds = {CATALOG.decode(i).kind for i in CATALOG.legal_ids(mask)}
    assert "PROPOSE_TRADE" in legal_kinds
